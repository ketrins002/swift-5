...
class Men: Person {
    ...
    init(name: String, age: Int, hasFacialHair: Bool) {
        self.facialHair = hasFacialHair
        super.init(name: name, age: age)
    }

    func introduce() {
        print("Привет, меня зовут \(name). Мне \(age) лет. Я мужчина.")
    }
}

class Women: Person {
    var likesShopping: Bool

    init(name: String, age: Int, enjoysShopping: Bool) {
        self.likesShopping = enjoysShopping
        super.init(name: name, age: age)
    }

    func introduce() {
        print("Привет, меня зовут \(name). Мне \(age) лет. Я женщина.")
    }
}

// Пример использования:
let john = Men(name: "Иван", age: 23, hasFacialHair: true)
john.introduce()

let anna = Women(name: "Алина", age: 25, enjoysShopping: true)
anna.introduce()

// 2. Создать класс *House* в нем несколько свойств - *width*, *height* и несколько методов: *create*(выводит площадь),*destroy*(отображает что дом уничтожен)
class House {
    var width: Double
    var height: Double
...
var studentArray: [Student] = [
    Student(name: "Алина", age: 18, averageScore: 4.5),
    Student(name: "Борис", age: 20, averageScore: 4.0),
    Student(name: "Степан", age: 19, averageScore: 3.3),
    Student(name: "Анастасия", age: 20, averageScore: 4.75)
]
